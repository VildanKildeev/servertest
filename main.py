...
# --- Database setup ---
# Предполагается, что 'database' и 'work_request_offers' импортированы корректно
    ...
    return [
        {
            ...
            "performer": {**offer, "hashed_password": None}
        }
        for offer in offers
    ]
...
# ----------------------------------------------------
# --- Machinery, Tool, Material Endpoints ---
# ----------------------------------------------------
...
# ----------------------------------------------------
# --- Static Files Mounting (Финальное исправление) ---
# ----------------------------------------------------
app.include_router(api_router)

# 1. Монтируем папку 'static'. Uvicorn ищет ее относительно корневой папки проекта.
app.mount("/static", StaticFiles(directory="static"), name="static")

# 2. Обслуживаем index.html с корневого пути.
@app.get("/", include_in_schema=False)
async def serve_app():
    # FileResponse использует относительный путь 'static/index.html'
